...
const WEIGHTS: Record<Lang, number> = {
  en: 0.40,
  ru: 0.20,
  es: 0.15,
  hi: 0.20,
...
async function fetchYouTubeVideos(lang: string, keywords: string[], apiKey: string): Promise<string[]> {
  ...
  for (const keyword of keywords) {
    const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=6&q=${encodeURIComponent(keyword)}&relevanceLanguage=${lang}&key=${apiKey}`;
...
export default {
  async fetchAndProduce(queue: Queue, env: any) {
    ...
    for (const lang of langs) {
      ...
      const perVideo = Math.min(accountsCount, MAX_COMMENTS_PER_VIDEO);
      ...
      for (const videoId of selected) {
        const accountIndexes = Array.from({ length: accountsCount }, (_, i) => i).sort(() => Math.random() - 0.5).slice(0, perVideo);
